...
try {
    ...
    app.log.warn(`Static files setup failed: ${String(error?.message || error)}`);
}
// Types and schemas
...
async function getUserFromRequest(request) {
    const authHeader = request.headers.authorization;
    if (!authHeader || !SUPABASE_URL || (!SUPABASE_ANON_KEY && !SUPABASE_SERVICE_KEY))
        return null;
    try {
        const token = authHeader.startsWith('Bearer ') ? authHeader.slice(7) : authHeader;
...
app.get('/api/auctions', async (request, reply) => {
    ...
    try {
        ...
        app.log.error(`Failed to fetch auctions: ${String(error?.message || error)}`);
...
app.get('/api/auctions/:id', async (request, reply) => {
    ...
    try {
        ...
        app.log.error(`Failed to fetch auction: ${String(error?.message || error)}`);
...
app.get('/api/auctions/:id/bids', async (request, reply) => {
    ...
    try {
        ...
        app.log.error(`Failed to fetch bids: ${String(error?.message || error)}`);
            ...
            try {
                ...
                if (toLive && toLive.length > 0) {
                    const { error: updErr } = await supabase
                        .from('auctions')
                        .update({ status: 'live' })
                        .in('id', toLive.map((a) => a.id));
                    if (updErr)
                        throw updErr;
                    for (const a of toLive) {
                ...
                app.log.warn(`Failed to activate scheduled auctions: ${String(e?.message || e)}`);
            }
