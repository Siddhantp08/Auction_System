...
export async function listBids(id, filters) {
    if (!supa)
        return { status: 200, body: { items: [] } };
...
export async function placeBid(auctionId, userId, amount) {
    ...
    if (e3)
        return { status: 500, body: e3.message };
    const notify = [];
    ...
    const prev = (top2 || []).find((b) => b.bidderId !== userId);
    ...
    if (allBidders && allBidders.length) {
        const unique = Array.from(new Set(allBidders.map((b) => b.bidderId)));
        ...
        for (const uid of recips)
            notify.push({ userId: uid, type: 'bid:update', payload: { auctionId, amount, title: a.title } });
    }
